...
class ButtonHandler:
    ...
    def ProcessAction(self, actiontype, actiondefs):
        if actiontype == "BRIGHTNESS_DECREASE" or actiontype == "BRIGHTNESS_INCREASE":
            ...
            for actiondef in actiondefs:
                ...
                if actiontype == "REST_POST":
                    api = config.THEME_DATA.get("API", {}).get(actiondef.get("API", {}), {})
                    url = api.get("URL", {}) + actiondef.get("URL", {})
                    headers = api.get("HEADERS", {})
                    json = actiondef.get("JSON_PAYLOAD", {})
                    post(url, headers = headers, json = json)
                    print("Rest Post " + url + " - " + str(json) + ". Headers " + str(headers))
                    #client.publish(actiondef['TOPIC'], actiondef['PAYLOAD'])
                            ...
                            if 'ACTION' in menu_data['B' + event.name]:
                                ...
                                for actiontype, actiondefs in actions.items():
                                    if actiontype == "SUBMENU":
                                        ...
                                        self.ProcessAction(actiontype, actiondefs)

                        if 'H' + event.name.upper() in config.THEME_DATA['HOTKEY']:
                            if 'ACTION' in config.THEME_DATA['HOTKEY']['H' + event.name.upper()]:
                                actions = config.THEME_DATA['HOTKEY']['H' + event.name.upper()]['ACTION']
                                for actiontype, actiondefs in actions.items():
                                    self.ProcessAction(actiontype, actiondefs)                                         
